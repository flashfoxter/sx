...
rc_ty sx_hashfs_volume_mod(sx_hashfs_t *h, const char *volume, const char *newname, const char *newowner, int64_t newsize, int max_revs, int modify_meta) {
    rc_ty ret = FAIL_EINTERNAL, s;
    const sx_hashfs_volume_t *vol = NULL;
    sx_uid_t newid;

    if(!volume || !newowner || !newname) {
        NULLARG();
        return ret;
    }
    ...
    if(*newname && strcmp(newname, vol->name)) {
        ...
        /* Check for the existence of a volume with new name is preformed after parsing,
         * but this call would also fail because of volume name constraints, so the existence check is skipped here. */
