...
static rc_ty alldb_2_1_5_to_2_1_6(sxi_all_db_t *alldb)
{
    ...
    do {
        ...
        snprintf(qry, sizeof(qry), "SELECT job, type, data FROM jobs WHERE complete = 0 AND job > :prev AND type IN (%d, %d, %d, %d, %d, %d) ORDER BY job LIMIT 1",
                JOBTYPE_REVSCLEAN, JOBTYPE_MASSDELETE, JOBTYPE_MASSRENAME, JOBTYPE_VOLUME_ACL, JOBTYPE_DELETE_VOLUME, JOBTYPE_MODIFY_VOLUME);
