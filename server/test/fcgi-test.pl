...
test_get "listing tiny$vol files", admin_only(200, 'application/json'), "tiny$vol?filter=*", undef,
    sub {
        my $json_raw = shift;
        print "\n", "JSON: ", $json_raw, "\n";
        my $json = get_json($json_raw) or return 0;
        return 0 unless is_hash($json->{'fileList'}) && scalar keys %{$json->{'fileList'}} == 1;
        return 0 unless is_hash($json->{'fileList'}->{'/toobig'});
    };
test_put_job "tiny$vol rename to different$vol", admin_only(200), "tiny$vol?o=mod", "{\"name\":\"different$vol\"}";
test_get "listing different$vol files", admin_only(200, 'application/json'), "different$vol?filter=*", undef,
    sub {
        my $json_raw = shift;
        print "\n", "JSON: ", $json_raw, "\n";
        my $json = get_json($json_raw) or return 0;
